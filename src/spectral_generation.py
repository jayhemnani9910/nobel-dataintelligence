...
class SpectralGenerator:
    ...
    def generate_dos(
        ...
        intensity_weighting: np.ndarray | None = None,
    ) -> np.ndarray:
        """
        ...
        if intensity_weighting is None:
            ...
            intensity_weighting = (
                intensity_weighting / max_w if max_w > 0 else np.ones_like(frequencies)
            )

        # Add Lorentzian peak for each mode
        gamma = broadening
        for freq, intensity in zip(frequencies, intensity_weighting, strict=True):
        ...
        logger.debug(
            f"Generated DOS spectrum with {len(frequencies)} modes "
    ...
    def generate_raman_spectrum(
        ...
        return self.generate_dos(
            frequencies, broadening=broadening, intensity_weighting=collectivities
        )
