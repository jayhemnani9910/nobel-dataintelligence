...
class ANMAnalyzer:
    ...
    def __init__(self, structure, cutoff: float = 15.0, distance_weighted: bool = True):
        """
    ...
    def compute_modes(self, k: int = 100) -> tuple[np.ndarray, np.ndarray]:
        ...
        if eigvals is None or eigvecs is None:
            raise RuntimeError(
                "ProDy returned no eigenvalues/eigenvectors. Did mode calculation succeed?"
            )
        ...
        if nonzero_idx.size == 0:
            raise RuntimeError(
                "All ANM eigenvalues are (near-)zero; cannot compute vibrational modes."
            )
    ...
    def get_residue_fluctuations(self, k: int = 100) -> np.ndarray:
        ...
        if (
            ...
            or self._eigenvectors.shape[1] < k
        ):
            self.compute_modes(k=k)
