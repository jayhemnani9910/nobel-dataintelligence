...
class VibroStructuralModel(nn.Module):
    ...
    def __init__(
        ...
        self.fusion = VibroStructuralFusion(
            latent_dim=latent_dim, fusion_type=fusion_type, dropout=dropout
        )
    ...
    def forward(
        ...
        if global_features is not None:
            ...
            global_proj = torch.zeros(fused_emb.size(0), 32, device=fused_emb.device)
        ...
        if task == "novozymes":
            ...
            if taxon_ids is not None:
                ...
                taxon_emb = torch.zeros(fused_emb.size(0), 1, device=fused_emb.device)
